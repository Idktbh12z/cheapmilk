...
local function start()
    ...
    workspace.ServerStuff.dropAmmo:FireServer("scrap", 0)
    _G.connection[5] =
        workspace.Interactables.ChildAdded:Connect(
        function(instance)
            if instance.Name == "Salvage Pack" then
                repeat
                    RunService.Heartbeat:Wait()
                until instance:FindFirstChild("stats") ~= nil
    ...
    function(input, gameProcessedEvent)
        ...
        if localPlayer.Character ~= nil then
            if input.KeyCode == Enum.KeyCode.R then
                if
                    localPlayer.Character:FindFirstChild("hasPrimary") ~= nil
                 then
