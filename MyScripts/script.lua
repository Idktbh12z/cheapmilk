...
for _, instance in pairs(workspace.Interactables:GetChildren()) do
    if instance.Name == "Barrel" then
        workspace.ServerStuff.detBarrel:FireServer(instance)
    end
end
    ...
    function(input, gameProcessedEvent)
        ...
        if localPlayer.Character ~= nil then
            if input.KeyCode == Enum.KeyCode.Minus then
                ...
                local originalPerk = localPlayer.Character:FindFirstChild("current_perk").Value
                if originalPerk ~= nil and originalPerk ~= "trapper" then
                ...
                local airstrike =
                    game.Players.LocalPlayer.Character.Humanoid:LoadAnimation(
                    game.ReplicatedStorage.animationSets.TPanimSets.special.radio
                )
                airstrike:Play()
                airstrike:AdjustSpeed(0.60)
                wait(3.85)
                local args = {
                    [1] = "throwticks",
                    [2] = {
                        [1] = CFrame.new(
                            Vector3.new(ROOT.CFrame.x + 10, ROOT.CFrame.y, ROOT.CFrame.z),
                            Vector3.new(ROOT.CFrame.Position)
                        ),
                        [2] = true
                    },
                    [3] = _G.serverKey,
                ...
                local args = {
                    ...
                    [2] = {
                        [1] = CFrame.new(
                            Vector3.new(ROOT.CFrame.x, ROOT.CFrame.y, ROOT.CFrame.z + 10),
                            Vector3.new(ROOT.CFrame.Position)
                        ),
                        [2] = true
                    },
                ...
                local args = {
                    ...
                    [2] = {
                        [1] = CFrame.new(
                            Vector3.new(ROOT.CFrame.x + 10, ROOT.CFrame.y, ROOT.CFrame.z - 10),
                            Vector3.new(ROOT.CFrame.Position)
                        ),
                        [2] = true
                    },
